...
def train(batch_size=0,bplus=0,random_seed=0,initial_batch_list = [],last_evaluation_episodes=0,load=False,save=False,um=1):
    ...
    if um == 0:
        env = ENV_ppo(length, sf, nagents, um = False)
    else:
        env = ENV_ppo(length, sf, nagents, um = True)
    ...
    lr_actor = 0.008       # learning rate for actor network
    lr_critic = 0.08       # learning rate for critic network
    ...
    while time_step <= max_training_timesteps:
        ...
        for t in range(1, max_ep_len+1):
            ...
            state, reward, done, eval_set1, eval_set2, metrics = env.step(actions, eval_set1 = eval_set1, eval_set2 = eval_set2)
...
if __name__ == "__main__":
    ...
    for i in range(arglist.idx*arglist.step,(arglist.idx+1)*arglist.step):
        ...
        pickle.dump(rewards_list,open("results/r_mappo_%d_%d.pkl"%(arglist.um,arglist.idx),"wb"))
